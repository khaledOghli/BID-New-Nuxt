...
<template>
  <header
    class="fixed top-0 z-40 w-full flex flex-none border-b border-gray-950/10 bg-white/[0.5] backdrop-blur-md backdrop-filter transition-colors duration-300 lg:z-50 dark:border-gray-50/[0.2] dark:bg-gray-950/[0.5]"
  >
    <!-- content -->
    <div
      class="mx-auto max-w-screen-2xl flex flex-1 items-center justify-between px-4"
    >
      <!-- title -->
      <div>
        <slot name="title">
          <NuxtLink to="/" class="text-lg text-primary-500 font-bold">
            <Icon
              name="simple-icons:nuxtdotjs"
              class="mr-2 inline-block text-xl font-black font-mono"
            />
      ...
      >
        ...
        <!-- others -->
        <div class="flex pl-4 text-xl space-x-3">
    ...
    >
      <AwesomeActionSheetGroup>
        ...
        <AwesomeActionSheetItem>
          <div
            class="flex flex-col items-center text-center text-sm divide-y divide-gray-400 dark:divide-gray-700"
          >
                ...
                <div :key="i" class="w-full">
                  <HeadlessDisclosure v-slot="{ open }">
                    <HeadlessDisclosureButton
                      :key="i"
                      class="w-full flex items-center justify-center py-2 text-gray-700 transition-all duration-300 dark:text-gray-300" :class="[
                        open ? 'font-bold' : '',
                      ]"
                    >
                      <span>{{
      ...
      <AwesomeActionSheetGroup>
        <AwesomeActionSheetItemButton
          class="flex items-center justify-center text-base space-x-2"
        >
